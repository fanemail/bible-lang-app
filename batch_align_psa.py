...
AUDIO_DIR       = r"E:\bible-lang-app\bible_data\audio\en\PSA"
OUTPUT_DIR      = r"E:\bible-lang-app\bible_data\timestamps\en\PSA"
DEVICE          = "cpu"
LANGUAGE        = "en"
...
def main():
    ...
    for ch in range(1, total + 1):
        ...
        if os.path.exists(out_path):
            print(f"[{ch:3d}/150] 跳過（已存在）：{ch:03d}.json")
            skipped += 1
            continue
        ch_key = str(ch)
        if ch_key not in chapters:
            print(f"[{ch:3d}/150] !! 文字資料庫中找不到第{ch}章，跳過")
            failed += 1
            continue
        t0 = time.time()
        print(f"[{ch:3d}/150] 處理第{ch}章...", end=" ", flush=True)
        try:
