...
def parse_ebible_chapter(html):
    ...
    for div in main.find_all("div", recursive=False):
        ...
        if verse_span:
            ...
            # 用正則只取"V"後面連續的數字，忽略後面可能夾帶的希伯來語版本對照文字
            # （例如 "V8 Ps.49.9" 這種格式，只取"8"，丟掉"Ps.49.9"）
            m = re.match(r'V?(\d+)', vid)
            if not m:
                verse_span.decompose()
                continue  # 抓不到乾淨節號，跳過這個標記（極少數情況）
            vnum = m.group(1)
...
def main():
    ...
    anomalies = []  # 記錄異常節號，供事後檢查
    for n in range(1, TEST_CHAPTERS + 1):
        en_verses = checkpoint["en"].get(str(n), {})
        zh_verses = checkpoint["zh"].get(str(n), {})
        ja_verses = {str(k): v for k, v in ja_book.get(n, {}).items()}

        raw_keys = set(en_verses.keys()) | set(zh_verses.keys()) | set(ja_verses.keys())
        # 防呆：只保留純數字的節號，異常的記錄下來但不讓程式崩潰
        clean_keys = []
        for k in raw_keys:
            if k.isdigit():
                clean_keys.append(k)
            else:
                anomalies.append((n, k))
        all_vnums = sorted(clean_keys, key=lambda x: int(x))
    ...
    print(f"\n完成！輸出檔案：{OUTPUT_PATH}")
    if anomalies:
        print(f"\n【注意】發現 {len(anomalies)} 個異常節號，已跳過（不影響其餘正常節號）：")
        for chap, bad_key in anomalies:
            print(f"  第{chap}章 節號異常：{bad_key!r}")
        print("這些異常節號對應的內容可能需要事後人工核對，把上面這份清單留著備查。")
